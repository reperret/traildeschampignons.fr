...
if($eventType=='Payment' && $eventState=='Authorized' && $itemState=='Processed')
{
    ...
    if($idRando!="")
    {
        ...
        $dbh->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $paiementRando=1;
        $reqUpdate = $dbh->prepare("UPDATE rando set helloTransactionRando=?,helloOrderidRando=?, paiementRando=? where idRando=?");
        $reqUpdate->bindParam(1,$payment_id);
        $reqUpdate->bindParam(2,$orderId);
        $reqUpdate->bindParam(3,$paiementRando);
        $reqUpdate->bindParam(4,$idRando);
        $reqUpdate->execute();
    ...
    else
    {
        ...
        $dbh->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $paiementEquipe=1;
        $reqUpdate = $dbh->prepare("UPDATE equipe set helloTransactionEquipe=?, helloOrderidEquipe=?, paiementEquipe=? where idEquipe=?");
        $reqUpdate->bindParam(1,$payment_id);
        $reqUpdate->bindParam(2,$orderId);
        $reqUpdate->bindParam(3,$paiementEquipe);
        $reqUpdate->bindParam(4,$idEquipe);
        $reqUpdate->execute();
